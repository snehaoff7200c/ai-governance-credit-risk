...
## What was configured
A `BigQuery Data Viewer` role was granted to the project owner's own account (`snehaoff7200c@gmail.com`) on the `credit_risk` dataset, intended to demonstrate read-only access provisioning for a hypothetical downstream consumer (e.g., a risk analyst who should be able to review governed data without modifying it).
